...
- **Content-driven pages**: new editorial copy belongs in `src/content/**` +
  `src/content.config.ts`, not hardcoded in `.astro`.
- **Schema alignment**: `src/content.config.ts` (Zod) and `.sitepins/schema/**`
...
Cite `path:line`. `src/pages/lab/**` is experimental — out of scope.
